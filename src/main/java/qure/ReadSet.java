...
public class ReadSet {
    ...
    public void estimateBaseSet() {
        ...
        for (int i = 0; i < population.size(); i++) {
            ...
            for (int j = 0; j < r.SNP_list.size(); j++) {
                ...
                if (bs.get((int)snp.position) != null) {
                    Base b = bs.get((int)snp.position);
                    switch (snp.base) {
                        case 'A':
                            b.A++;
                            break;
        ...
        for (Integer aKey1 : bs.keySet()) {
            Base b = bs.get(aKey1);
            for (Read r : population) {
                if (b.position >= r.start && b.position <= r.stop)
                    b.coverage++;
            }
        }
        java.util.Set<Integer> keySet = bs.keySet();
        LinkedList<Double> bc = new LinkedList();
        for (Integer aKey : bs.keySet()) {
            if (bs.get(aKey).coverage > 0)
                bc.add(bs.get(aKey).coverage);
        }
        ...
        Integer[] key = (Integer[])keySet.toArray();
        Arrays.sort(key);
    ...
    public class CorrectReadsThread implements Runnable {
        Read read;
        HashMap<Integer, Base> baseSet;

        public CorrectReadsThread(Read r, HashMap<Integer, Base> b) {
            read = r;
            baseSet = b;
        }
    ...
    public class RandomScoresThread implements Runnable {
        ...
        public RandomScoresThread(Vector<Read> p, double[] r, String ref, double a, double s, float go, float ge, Matrix m, int i) {
            population = p;
            rndScores = r;
            referenceGenome = ref;
            avgReadLength = a;
        ...
        public void run() {
            ...
            Alignment rndAlignment = SmithWatermanGotoh.align(rndQuerySeq, rndReferSeq, matrix, gop, gep);
    ...
    public void setRandomScoresParallel(int sampleSize, float gop, float gep, int n_proc) {
        ...
        while (i < sampleSize) {
            ...
            if (i == 0)
                for (int j = 0; j < n_proc; j++) {
                    Runnable runnable = new RandomScoresThread(population, rndScores, referenceGenome, avgReadLength, stdReadLength, gop, gep, matrix, i);
                    thread_list[j] = new Thread(runnable);
                    thread_list[j].start();
                    i++;
                }
    ...
    public void setPopulation(Iterable<SAMRecord> records, DNASequence reference) {
        ...
        for (SAMRecord record : records) {
            ...
            if (read.stop > alignStop)
                alignStop = read.stop;
        }
        alignStart = Math.max(0.0, alignStart);
        double[] rl = new double[population.size()];
        for (int i = 0; i < population.size(); i++) {
            population.get(i).adjustSequence();
    ...
    public class randomAmpliconThread implements Runnable {
        ...
        public void run() {
            ...
            //this will notify main thread that we have finished
            synchronized (lock) {
                lock.notify();
            }
        }
    ...
    private String printEstimateAmpliconTime(String perc, int t, int runs) {
        if (perc.length() != 0) {
            for (int i = 0; i < perc.length(); i++)
                System.out.print("\b");
        }
        String newPerc = (Functions.roundToDecimals(100f * ((float) (t)) / ((float) (runs)), 2)) + "%  ";
        System.out.print(newPerc);
        return newPerc;
    }

    public void estimateAmpliconsParallel(int runs, int n_proc) throws InterruptedException {
        final Vector<Read> pop = new Vector<>();
        ...
        double[] windowSizes = {(avgReadLength + stdReadLength), avgReadLength, (avgReadLength - stdReadLength),
                (avgReadLength - 2 * stdReadLength), (avgReadLength - 3 * stdReadLength), avgReadLength * 5 / 6d, avgReadLength * 4 / 5d,
                avgReadLength * 3 / 4d, avgReadLength * 2 / 3d, avgReadLength * 1 / 2d, avgReadLength * 1 / 3d, avgReadLength * 1 / 4d, avgReadLength * 1 / 5d,
                avgReadLength * 1 / 6d};

        for (int count = 0; count < 5; count++) {
            if (windowSizes[count] < 0)
                windowSizes[count] = Math.min(MIN_READ_LEN, avgReadLength - 5 * (count + 1));
        }
        ...
        Executor executor = Executors.newFixedThreadPool(n_proc);
        CompletionService<AmpliconSet> completionService = new ExecutorCompletionService<AmpliconSet>(executor);

        for (int t = 0; t < runs; t++) {
            completionService.submit(new Callable<AmpliconSet>() {
                @Override
                public AmpliconSet call() throws Exception {
                    AmpliconSet amplicon = new AmpliconSet();
                    amplicon.initialiseRandomAmpliconSet(avgReadLength, stdReadLength, alignStart, alignStop, pop);
                    int il = 0;
                    while (il < 50) {
                        if (amplicon.checkConsistency() && amplicon.minReadCoverage > 1)
        ...
        String perc = printEstimateAmpliconTime("", 0, runs);
        for (int t = 0; t < runs; t++) {
            Future<AmpliconSet> resultFuture = completionService.take();
            try {
                AmpliconSet ampSet = resultFuture.get();
                if (ampSet.checkConsistency() && ampSet.minReadCoverage > 1) {
                    ampliconList.add(ampSet);
                }
                perc = printEstimateAmpliconTime(perc, t, runs);
            } catch (ExecutionException ee) {
                System.err.println("Error getting random amplicon set");
            }
        }

        System.out.print("100%  ");
        System.out.println();
        System.out.println("\tphase 2: assessing best a-posteriori overlaps set");
        ...
        for (int oo = 0; oo < bestAmplicon.starts.length; oo++) {
            System.out.print("\t\t\t" + (int)bestAmplicon.starts[oo] + "-" + (int)bestAmplicon.stops[oo] + "\r\n");
        }
